...
import { UserCheck, ShoppingBag, ShieldCheck, CheckCircle2, AlertCircle, ArrowRight } from 'lucide-react';
...
export const CustomerDashboard = () => {
  ...
  const handleTestProtectedApi = async () => {
    ...
    setTestError('');
    try {
      const res = await authService.testCustomerAccess();
      setApiTestResult(res);
    } catch (err) {
      setTestError(err.message || 'Failed to call customer protected API');
    } finally {
      setTestingApi(false);
    }
  };
    ...
    <div style={{ backgroundColor: 'var(--bg-app)', minHeight: '80vh', padding: '3.5rem 0' }}>
      <div className="container" style={{ maxWidth: '880px' }}>
        ...
        <div className="card" style={{
          ...
          marginBottom: '2rem',
          borderLeft: '6px solid var(--primary-700)'
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1.25rem' }}>
            ...
            <div>
              <span className="badge badge-category">Active Customer Session</span>
          ...
          <p style={{ fontSize: '0.9375rem', color: 'var(--text-body)', lineHeight: '1.6', marginBottom: '1.5rem' }}>
            You are securely authenticated via <strong>Spring Security & JWT</strong> with the <code>CUSTOMER</code> role. You can browse verified grower crops, manage your fresh orders, and contact local farmers.
          </p>
          ...
          <div style={{
            ...
            <div>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: '600', textTransform: 'uppercase' }}>Email Address</span>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.9375rem', fontWeight: '700', color: 'var(--primary-800)' }}>{role}</p>
            </div>
            <div>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: '600', textTransform: 'uppercase' }}>Account Status</span>
        ...
        <div className="card" style={{
          ...
          <h2 style={{ fontSize: '1.25rem', fontWeight: '800', color: 'var(--primary-900)', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            Spring Security Token RBAC Test
          </h2>
          <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '1.25rem' }}>
            Click below to execute an authenticated <code>GET /api/customer/test</code> request using your Bearer JWT token to verify backend authorization.
          </p>
          ...
          >
            {testingApi ? 'Verifying with Spring Boot...' : 'Verify Customer Protected Endpoint'}
          </Button>
        ...
        {/* Quick Produce Catalog Navigation */}
        <div style={{ textAlign: 'center' }}>
          <Link to="/products">
            <Button variant="primary" size="lg" icon={<ShoppingBag size={18} />}>
              Explore Fresh Produce Catalog
            </Button>
          </Link>
        </div>
